...
const ATTACHMENTS = Array.from({ length: 20 }, (_, index) => _mock.image.cover(index));
...
const ASSIGNEES = Array.from({ length: 20 }, (_, index) => ({
  id: _mock.id(index),
...
const tasks = Array.from({ length: 6 }, (_, index) => {
  ...
  const attachments =
    (index === 1 && ATTACHMENTS.slice(11, 15)) ||
    (index === 5 && ATTACHMENTS.slice(4, 9)) ||
    [];
  ...
  const priority =
    ([1, 3].includes(index) && 'hight') ||
    ([2, 4].includes(index) && 'medium') ||
    'low';
